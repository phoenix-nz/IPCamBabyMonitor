...
public class EasyNAudioSender extends Thread {
    ...
    public EasyNAudioSender(OutputStream stream) {
    	super();
    	this.stream = stream;
//    	Buffer more than required - we seem to be relatively slow
	...
	public void run() {
		...
		while(running) {
            ...
            if(audio != null) {
            	byte[] frameData = new byte[EasyNUtil.HeaderSize + AUDIO_PACKET_SIZE];
            	for (int i = 0; i < 4; i++)                 //preamble
            		frameData[i] = EasyNUtil.MO_V[i];
            	frameData[15] = frameData[19] = (byte) 0xb1;
            	frameData[EasyNUtil.CommandPos] = 3;
            	// We have to create all of the preamble
            	// First four bytes = timestamp
            	audioPos = EasyNUtil.HeaderSize;
